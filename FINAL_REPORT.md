...
- **Живое демо:** https://economics.toxick.ru (Docker + nginx на VPS клиента, рядом с
  существующим приложением — отдельный стек, порт `127.0.0.1:8080`, БД без публикации;
  поддомен интегратора, сертификат Let's Encrypt). Переключатель ролей слева внизу.
...
- **Мобильная адаптивность:** боковое меню на iPhone/узких экранах — выезжающая
  панель, а не фиксированные 256px; формы, KPI и панели сворачиваются в одну колонку;
  без горизонтального скролла страницы (ТЗ desktop-приоритет сохранён, iframe просто
  не ломается на телефоне).
- **Защита кода:** лицензия MIT → проприетарная «только для оценки»; белый список
  порталов (`ALLOWED_PORTAL_MEMBER_IDS`/`ALLOWED_PORTAL_DOMAINS`) отклоняет установку/
...
- **Публикация в Маркетплейс Bitrix24 и автоматический биллинг** — не сделаны: нужна
  регистрация партнёром, модерация Bitrix, и (для оплаты картой) `app.info`/
  `ONAPPPAYMENT` или свой эквайринг. Текущий ручной биллинг (`/operator`) — рабочая
  замена на переходный период, архитектурно совместимая с автоматизацией позже
  (см. `docs/DECISIONS.md`, ADR-015/016).
