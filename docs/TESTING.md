...
| `projects/plan-gating` | FREE: без лимита проектов; импорт из CRM → 403; PRO: лимита нет, импорт проходит гейт; истёкший Pro-грант = снова FREE |
...
| `billing` (unit) | `createProLeadSchema`: контакт от 3 символов, комментарий опционален, обрезка пробелов, лимит длины |
| `billing` (integration) | заявка на Pro: нет заявки → `pending:false`; повторная заявка при открытой → 409; неавторизованный → 401; оператор видит лид с доменом портала; «Связался»/«Отказ»/выдача Pro с автопереводом в CONVERTED; после конверсии портал может подать новую заявку; `overview` считает порталы/Pro/новые лиды |
| `onboarding` | свежий портал — все 3 шага не выполнены; шаги переключаются по факту данных (проект → команда → операция), `complete` только когда все три; скрыть чек-лист — 403 для EMPLOYEE, 200 для MANAGER, пишет `onboardingDismissedAt` |
