...
| `ALLOWED_PORTAL_MEMBER_IDS` / `ALLOWED_PORTAL_DOMAINS` | белый список порталов (через запятую). Если задан — установить и открыть приложение может только портал из списка, остальные получают отказ. Пусто — проверка выключена. Демо-портал разрешён всегда |
| `OPERATOR_EMAIL` / `OPERATOR_PASSWORD_HASH` | учётная запись `/operator` — панели, где вы вручную выдаёте тариф Pro оплатившим порталам. Хеш — через `node scripts/hash-operator-password.mjs '<пароль>'` |
...
> **Ограничение установки.** Задайте `ALLOWED_PORTAL_MEMBER_IDS` (и/или
> `ALLOWED_PORTAL_DOMAINS`) — тогда установить и открыть приложение сможет только
> портал из этого списка. `member_id` портала виден в payload обработчика установки
> и в разделе «Разработчикам». Это защита от несанкционированного развёртывания
> в дополнение к лицензии (см. `LICENSE`).
...
Биллинг сейчас ручной — оператор (владелец приложения) выдаёт Pro тем, кто оплатил вне
приложения (перевод/договорённость); Marketplace-платежи можно подключить позже поверх
той же модели (`PortalInstallation.plan`, `PortalInstallation.planExpiresAt`).

| | Free | Pro |
|---|---|---|
| Активные проекты | до 3 | без лимита |
| Импорт проекта из сделки/компании Bitrix24 | — | ✅ |
| Всё остальное (доход/расход, статьи, история, дашборд) | ✅ | ✅ |

Гейты — только на backend (`src/lib/billing/plan.ts`, `requirePro(...)`); фронт лишь
показывает заглушку с призывом оформить Pro. Просроченный Pro-грант (`planExpiresAt` в
прошлом) автоматически считается Free — без ручного отзыва. Демо-портал всегда видит
полный функционал.

### `/operator` — панель оператора

Отдельный вход, не через Bitrix24 (своя cookie-сессия, email+пароль из `.env`, не путать
с ролями ADMIN/MANAGER/EMPLOYEE):

1. Сгенерировать хеш пароля: `node scripts/hash-operator-password.mjs 'мой-пароль'`
2. Вставить `OPERATOR_EMAIL` и полученный `OPERATOR_PASSWORD_HASH` в `.env` (или на
   сервере, где крутится прод), перезапустить.
3. Открыть `https://<APP_URL>/operator/login`.
4. Список всех порталов (домен/member_id, план, дата истечения, число проектов/
   сотрудников), поиск, разворачиваемая карточка — выдать/отозвать Pro с датой окончания
   (или бессрочно) и заметкой. Каждое изменение пишется в историю выдач.

## Лицензия

Проприетарная, **только для оценки** в рамках тестового задания — см. [`LICENSE`](LICENSE).
Развёртывание, установка на портал Bitrix24 и любое использование сверх ознакомления —
только с письменного разрешения правообладателя. Это не open source.
