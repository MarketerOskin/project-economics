...
> **Живое демо:** https://economics.toxick.ru — открывается сразу, переключатель ролей
> (Администратор / Руководитель / Сотрудник) слева внизу. Данные демо-портала уже
> загружены (7 проектов разной экономики). Поддомен и TLS-сертификат (Let's Encrypt)
> подключены — обработчик и установка доступны по HTTPS, готово для подключения
> к Bitrix24.
...
9. События: приложение само подписывается на `ONAPPUNINSTALL` (`event.bind` на
   `https://<APP_URL>/api/bitrix/events`) при установке. Если пользователь удаляет приложение
   с опцией «Очистить данные приложения» (`data[CLEAN]=1`), все данные портала стираются;
   иначе сбрасываются только токены. Журнал REST-вызовов (`ApiCallLog`) хранится 7 дней.
...
| | Free | Pro |
|---|---|---|
| Активные проекты | без лимита | без лимита |
...
Для свежего реального портала (не демо) первый экран — не пустой дашборд, а чек-лист
«Начало работы»: создать проект → добавить команду → внести первую операцию. Каждый
шаг проверяется по фактическим данным (не отмечается руками), чек-лист сам исчезает,
когда всё готово, либо скрывается по ✕ — решение сохраняется на портал.

### `/pricing` — тарифы и заявка на Pro

Виден всем ADMIN/MANAGER (пункт меню «Тарифы»), а также по ссылке из `/settings` и из
любого `UpsellNotice`. Сравнение Free/Pro, кнопка «Оформить Pro» открывает форму
(контакт + комментарий) → создаёт `ProLead`. Пока заявка открыта — повторная отправка
даёт понятную ошибку, а не тихий дубль.
...
4. Вкладка **«Порталы»**: список всех порталов (домен/member_id, план, дата истечения,
   число проектов/сотрудников), поиск, разворачиваемая карточка — выдать/отозвать Pro с
   датой окончания (или бессрочно) и заметкой. Каждое изменение пишется в историю выдач.
5. Вкладка **«Заявки на Pro»** (с счётчиком новых): все заявки с `/pricing`, домен
   портала, контакт, комментарий. «Выдать Pro» — грант и перевод заявки в «Оплатил»
   одним кликом; «Связался» / «Отказ» — для промежуточных статусов.
