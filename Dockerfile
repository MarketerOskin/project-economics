...
#
# Uses the full node:22-bookworm image (openssl + ca-certificates already inside) so the
# build never needs apt-get — some hosts' BuildKit sandbox can't reach the Debian mirrors.
